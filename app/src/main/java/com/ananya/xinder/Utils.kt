...
import com.ananya.xinder.entities.Profile
...
object Utils {
    ...
    fun loadProfiles(context: Context): List<Profile>? {
        return try {
            ...
            val array =
                    JSONArray(loadJSONFromAsset(context, "profiles.json"))
            val profileList: MutableList<Profile> =
                    ArrayList()
            for (i in 0 until array.length()) {
                val profile =
                        gson.fromJson(array.getString(i), Profile::class.java)
